...
class Endereco(Resource):
  ...
  db_string = "postgresql://postgres:postgres@postgres@localhost:5432/imobiliaria"
...
class Bancos(Resource):
  ...
  def get(self, id_banco):
    try:
      ...
      #Executa a leitura dos parâmetros da tabela
      seleciona_declaracao = self.tab_bancos.select()
      resulta = conn.execute(seleciona_declaracao)
  ...
  def post(self, id_banco):
    try:
      ...
      with self.db.connect() as conn:
        insere_declaracao = self.tab_bancos.insert().returning(self.tab_bancos.c.id_imovel).values(nome_banco=nome_banco)
        resultado = conn.execute(insere_declaracao)
        return jsonify({'resultado': [dict(row) for row in resultado]})
...
@app.doc(responses={ 200: 'OK', 400: 'Argumento Inválido', 500: 'Mapeando a chave de erro' }, params={ 'id_tipo': 'Especifica o Id associado com o banco' })
@app.expect(model_banco)             
def delete(self, id_banco):
  try:
    ...
    with self.db.connect() as conn:
      atualiza_declaracao = self.tab_bancos.delete().returning(self.tab_bancos.c.id_banco).values(nome_banco=nome_banco)
      results = conn.execute(atualiza_declaracao)
      return jsonify({'resultado': [dict(row) for row in results]})
  except KeyError as e:
    name_space.abort(500, e.__doc__, status = "Informação não pode ser recuperada", statusCode = "500")
